...
class GRunView extends WatchUi.DataField {
  ...
  function configureAreaHeight() {
    ...
    var typeArray = [
      vType[0],
      vType[1] + vType[2] + vType[3],
      vType[4] + vType[5] + vType[6],
      vType[7] + vType[8],
      vType[9],
    ];
    ...
    for (var i = 0; i < 5; i++) {
      ...
      if (i == 0) {
        assignAreaHeightValues(0, 1, y, height);
      } else if (i == 1) {
        assignAreaHeightValues(1, 4, y, height);
      } else if (i == 2) {
        assignAreaHeightValues(4, 7, y, height);
      } else if (i == 3) {
        assignAreaHeightValues(7, 9, y, height);
      } else if (i == 4) {
        assignAreaHeightValues(9, 10, y, height);
      }
  ...
  function configureAreaWidth(s, beginIndex, endIndex) {
    ...
    var totalWidthRatio = sumAreaArray(
      numberArray,
      vType.slice(beginIndex, endIndex)
    );

    var x = 0;
    for (var i = beginIndex; i < endIndex; i++) {
      var width = round(
        (deviceWidth.toFloat() * numberArray[i - beginIndex]) / totalWidthRatio
      );
  ...
  function initializeUserData() {
    ...
    var TYPE_DEFAULT_VALUE = [
      6 /* OPTION_CURRENT_HEART_RATE */, 50 /* OPTION_ETA_5K */,
      0 /* OPTION_EMPTY */, 7 /* OPTION_CURRENT_PACE */,
      5 /* OPTION_ELAPSED_DISTANCE */, 14 /* OPTION_CURRENT_CADENCE */,
      10 /* OPTION_AVERAGE_PACE */, 2 /* OPTION_TIMER_TIME */,
    ...
    for (var i = 0; i < 10; i++) {
      ...
      vType[i] = getParameter(
        "Area" + (i + 1).toString(),
        TYPE_DEFAULT_VALUE[i]
      );
    ...
    */
    ...
    if (headerHeight2 > 0 && headerHeight2 < headerHeight) {
  ...
  function initialize() {
    ...
    isDistanceUnitsImperial =
      deviceSettings.distanceUnits == System.UNIT_STATUTE;
    isElevationUnitsImperial =
      deviceSettings.elevationUnits == System.UNIT_STATUTE;
  ...
  function onTimerLap() {
    ...
    if (lapDistance > 0) {
      ...
      if (diff < lapDistance / 2) {
        distanceOffset -= diff;
      } // Distance higher than expected
      else {
        distanceOffset += lapDistance - diff;
      } // Distance lower than expected
    }
  ...
  function computeValue(info, id, value, valueData) {
    if (value <= 20) {
      ...
      if (
        value == 6 /* OPTION_CURRENT_HEART_RATE */ &&
      ...
      if (
        value == 9 /* OPTION_AVERAGE_HEART_RATE */ &&
      ...
      if (value == 10 /* OPTION_AVERAGE_PACE */ && info.averageSpeed != null) {
        ...
        if (info.averageSpeed <= 0) {
          return 0;
        }
        return convertUnitIfRequired(
          1000 / info.averageSpeed,
          1.609344 /* CONVERSION_MILE_TO_KM */,
          isPaceUnitsImperial
        );
      }
      ...
      if (value == 12 /* OPTION_AVERAGE_SPEED */ && info.averageSpeed != null) {
        if (lapDistance > 0 && distanceOffset != 0) {
          return timer <= 0 ? 0 : (distance / timer) * 3600;
        }
        return convertUnitIfRequired(
      ...
      if (
        value == 14 /* OPTION_CURRENT_CADENCE */ &&
        ...
        if (
          firmwareVersion[0] == 19 &&
          (firmwareVersion[1] == 20 || firmwareVersion[1] == 40)
        ) {
      ...
      if (
        ...
        if (valueData != info.currentLocationAccuracy) {
          ...
          imgGPS = WatchUi.loadResource(
            rezGPSTable[info.currentLocationAccuracy]
          );
        }
    ...
    if (
      value == 28 /* OPTION_TRAINING_EFFECT */ &&
      info.trainingEffect != null
    ) {
    ...
    if (
      value == 33 /* OPTION_AVERAGE_CADENCE */ &&
      info.averageCadence != null
    ) {
    ...
    if (value == 35 /* OPTION_HR_ZONE */ && info.currentHeartRate != null) {
      ...
      for (var i = 1; i < 6; i++) {
        if (hr < hrZones[i] || i == 5) {
          return (hr - hrZones[i - 1]) / (hrZones[i] - hrZones[i - 1]) + i;
        }
      }
    }

    if (
      value >= 50 /* OPTION_ETA_5K */ &&
      ...
      if (value <= 55 /* OPTION_ETA_100K */) {
        ...
        if (lapDistance > 0 && distanceOffset != 0) {
          if (timer > 0) {
            avgSpeed = distance / timer;
          }
        } else if (info.averageSpeed != null) {
          avgSpeed = convertUnitIfRequired(
            info.averageSpeed / 1000,
            0.62137119 /* CONVERSION_KM_TO_MILE */,
            isDistanceUnitsImperial
          );
        }
  ...
  function compute(info) {
    ...
    if (info.elapsedDistance != null) {
      distance = convertUnitIfRequired(
        (info.elapsedDistance + distanceOffset) / 1000.0,
        0.62137119 /* CONVERSION_KM_TO_MILE */,
        isDistanceUnitsImperial
      );
  ...
  function onUpdate(dc) {
    ...
    if (vType[7] != 0 /* OPTION_EMPTY */ && vType[8] != 0 /* OPTION_EMPTY */) {
      dc.setPenWidth(3);
      var fgColor = singleBackgroundColor
  ...
  function drawVerticalLine(dc, areaIndex) {
    if (vType[areaIndex] != 0 /* OPTION_EMPTY */) {
      dc.drawLine(
        vArea[areaIndex][0],
        vArea[areaIndex][1],
        vArea[areaIndex][0],
        vArea[areaIndex][1] + vArea[areaIndex][3]
      );
    }
  }
  ...
  function displayArea(
    ...
    if (id >= 8 && singleBackgroundColor == false) {
      isWhiteBG = !isWhiteBG;
    }
    ...
    if (id >= 2 && id <= 7) {
      ...
      if (headerHeight > 0) {
        ...
        var headerBgColor =
          dynamicHeaderBackgroundColor && color != null
            ? color
        ...
        if (headerY == 0) {
          headerY = 3;
          currentHeaderHeight -= 3;
        } // Padding Top
        if (headerY + currentHeaderHeight == deviceHeight) {
          currentHeaderHeight -= 3;
        } // Padding Bottom

        // Padding Left/Right
        if (areaX == 0) {
          paddingLeft = (deviceWidth - getWidth(headerY, headerHeight)) / 2;
        }
        if (areaX + areaWidth == deviceWidth) {
          paddingRight = (deviceWidth - getWidth(headerY, headerHeight)) / 2;
        }
        ...
        if (
          headerBgColor == Graphics.COLOR_RED ||
          headerBgColor == Graphics.COLOR_DK_GREEN
        ) {
          headerFgColor = Graphics.COLOR_WHITE;
        }
    ...
    if (color != null) {
      if (dynamicBackgroundColor) {
        ...
        if (bgColor == Graphics.COLOR_RED) {
          fgColor = Graphics.COLOR_WHITE;
        } else {
          fgColor = primaryForegroundColor;
        }
    ...
    if (areaY + areaHeight == deviceHeight) {
      areaHeight -= 3;
    } // Padding Bottom

    // Padding Left/Right
    if (areaX == 0) {
      paddingLeft = (deviceWidth - getWidth(areaY, areaHeight)) / 2;
    }
    if (areaX + areaWidth == deviceWidth) {
      paddingRight = (deviceWidth - getWidth(areaY, areaHeight)) / 2;
    }
    if (id == 8 && vType[8] != 0 /* OPTION_EMPTY */) {
      paddingRight += 4;
    } else if (id == 9 && vType[7] != 0 /* OPTION_EMPTY */) {
      paddingLeft += 4;
    }
    areaX += paddingLeft;
    areaWidth -= paddingLeft + paddingRight;
    ...
    if (type >= 18 && type <= 20) {
      ...
      if (fgColor != null) {
        dc.setColor(fgColor, Graphics.COLOR_TRANSPARENT);
      } else if (id < 8 || singleBackgroundColor) {
        dc.setColor(primaryForegroundColor, Graphics.COLOR_TRANSPARENT);
      } else {
        dc.setColor(
      ...
      if (displayIcon) {
        ...
        var textWidth =
          12 /*(iconWidth / 2)*/ +
          dc.getTextWidthInPixels(formattedValue, font);
        dc.drawText(
          areaXcenter - 12 /*(iconWidth / 2)*/ - textWidth / 2,
          areaYcenter,
          fontIcons,
          0,
          5 /* Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER */
  ...
  function getHeaderName(type) {
    if (type == 1 /* OPTION_CURRENT_TIME */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_CURRENT_TIME);
    }
    if (type == 2 /* OPTION_TIMER_TIME */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_TIMER_TIME);
    }
    if (type == 5 /* OPTION_ELAPSED_DISTANCE */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_ELAPSED_DISTANCE);
    }
    if (type == 6 /* OPTION_CURRENT_HEART_RATE */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_CURRENT_HEART_RATE);
    }
    if (type == 7 /* OPTION_CURRENT_PACE */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_CURRENT_PACE);
    }
    if (type == 8 /* OPTION_CURRENT_SPEED */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_CURRENT_SPEED);
    }
    if (type == 9 /* OPTION_AVERAGE_HEART_RATE */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_AVERAGE_HEART_RATE);
    }
    if (type == 10 /* OPTION_AVERAGE_PACE */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_AVERAGE_PACE);
    }
    if (type == 12 /* OPTION_AVERAGE_SPEED */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_AVERAGE_SPEED);
    }
    if (type == 13 /* OPTION_CALORIES */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_CALORIES);
    }
    if (type == 14 /* OPTION_CURRENT_CADENCE */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_CURRENT_CADENCE);
    }
    if (type == 15 /* OPTION_ALTITUDE */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_ALTITUDE);
    }
    if (type == 16 /* OPTION_TOTAL_ASCENT */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_TOTAL_ASCENT);
    }
    if (type == 17 /* OPTION_TOTAL_DESCENT */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_TOTAL_DESCENT);
    }
    if (type == 18 /* OPTION_CURRENT_BATTERY */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_CURRENT_BATTERY);
    }
    if (type == 19 /* OPTION_CURRENT_LOCATION_ACCURACY */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_CURRENT_LOCATION_ACCURACY);
    }
    if (type == 20 /* OPTION_CURRENT_LOCATION_ACCURACY_AND_BATTERY */) {
      return WatchUi.loadResource(
        Rez.Strings.OPTION_CURRENT_LOCATION_ACCURACY_AND_BATTERY
      );
    }
    if (type == 21 /* OPTION_CURRENT_POWER */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_CURRENT_POWER);
    }
    if (type == 22 /* OPTION_AVERAGE_POWER */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_AVERAGE_POWER);
    }
    if (type == 25 /* OPTION_CURRENT_LAP_TIME */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_CURRENT_LAP_TIME);
    }
    if (type == 26 /* OPTION_CURRENT_LAP_DISTANCE */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_CURRENT_LAP_DISTANCE);
    }
    if (type == 27 /* OPTION_CURRENT_LAP_PACE */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_CURRENT_LAP_PACE);
    }
    if (type == 28 /* OPTION_TRAINING_EFFECT */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_TRAINING_EFFECT);
    }
    if (type == 30 /* OPTION_TIMER_TIME_ON_PREVIOUS_LAP */) {
      return WatchUi.loadResource(
        Rez.Strings.OPTION_TIMER_TIME_ON_PREVIOUS_LAP
      );
    }
    if (type == 31 /* OPTION_ETA_LAP */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_ETA_LAP);
    }
    if (type == 32 /* OPTION_LAP_COUNT */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_LAP_COUNT);
    }
    if (type == 33 /* OPTION_AVERAGE_CADENCE */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_AVERAGE_CADENCE);
    }
    if (type == 34 /* OPTION_TIME_OFFSET */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_TIME_OFFSET);
    }
    if (type == 35 /* OPTION_HR_ZONE */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_HR_ZONE);
    }
    if (type == 50 /* OPTION_ETA_5K */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_ETA_5K);
    }
    if (type == 51 /* OPTION_ETA_10K */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_ETA_10K);
    }
    if (type == 52 /* OPTION_ETA_HALF_MARATHON */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_ETA_HALF_MARATHON);
    }
    if (type == 53 /* OPTION_ETA_MARATHON */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_ETA_MARATHON);
    }
    if (type == 54 /* OPTION_ETA_50K */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_ETA_50K);
    }
    if (type == 55 /* OPTION_ETA_100K */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_ETA_100K);
    }
    if (type == 56 /* OPTION_REQUIRED_PACE_5K */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_REQUIRED_PACE_5K);
    }
    if (type == 57 /* OPTION_REQUIRED_PACE_10K */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_REQUIRED_PACE_10K);
    }
    if (type == 58 /* OPTION_REQUIRED_PACE_HALF_MARATHON */) {
      return WatchUi.loadResource(
        Rez.Strings.OPTION_REQUIRED_PACE_HALF_MARATHON
      );
    }
    if (type == 59 /* OPTION_REQUIRED_PACE_MARATHON */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_REQUIRED_PACE_MARATHON);
    }
    if (type == 60 /* OPTION_REQUIRED_PACE_50K */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_REQUIRED_PACE_50K);
    }
    if (type == 61 /* OPTION_REQUIRED_PACE_100K */) {
      return WatchUi.loadResource(Rez.Strings.OPTION_REQUIRED_PACE_100K);
    }
    return "";
  }
  ...
  function getFormattedValue(id, type, value) {
    ...
    if (
      type == 5 /* OPTION_ELAPSED_DISTANCE */ ||
  ...
  function getFont(dc, value, maxWidth, maxHeight) {
    ...
    while (font > Graphics.FONT_XTINY) {
      ...
      if (textDimensions[0] <= maxWidth && textDimensions[1] <= maxHeight) {
        break;
      }
      font--;
    }
  ...
  function getWidth(y, height) {
    if (System.getDeviceSettings().screenShape == System.SCREEN_SHAPE_ROUND) {
      ...
      if (yCenter < radius) {
        heightCenter = height - heightCenter;
      }
  ...
  function formatDuration(seconds, displayHour) {
    ...
    return (
      prefix +
      (hh > 0 ? hh + ":" + mm.format("%02d") : mm.format("%d")) +
      ":" +
      ss.format("%02d")
    );
  }
  ...
  function drawBattery(dc, id, x, y, fixedBackground)
  {
    ...
    dc.drawText(x, y - 1, 0, round(batteryPercentage) + "%", Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
  }
  */
